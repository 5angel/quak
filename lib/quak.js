	...
	var View = function () {
	  ...
	  _createClass(View, [{
	    ...
	    value: function render(model) {
	      ...
	      var found = (0, _parse2.default)(container);
	
	      var _loop = function _loop(prop) {
	        _this._views[prop] = found[prop].map(function (_ref) {
	          var node = _ref.node;
	          var expr = _ref.expr;
	    ...
	    if (node.nodeType === 1) {
	      ...
	      try {
	        for (var _iterator = (0, _utils.toArray)(node.attributes)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	          ...
	          hasView = hasView || (0, _utils.contains)(SPECIAL, attr.name);
	
	          var chldn = children[attr.name] || [];
	          var match = attr.value.match(RE_EXPR);
	          ...
	          if (match) {
	            ...
	            if (hasView) {
	              ...
	              chldn.push({ node: node, expr: expr });
